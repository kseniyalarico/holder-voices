...
export default function NewPollPage() {
  ...
  function selectAllCollections() {
    setSelected(collections.map((c) => c.id));
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <fieldset className="flex flex-col gap-2 text-sm">
          <div className="mb-1 flex items-center justify-between">
            <legend>Participating collections</legend>
            <button
              type="button"
              onClick={selectAllCollections}
              disabled={collections.length === 0}
              className="rounded-md border border-border px-2.5 py-1 text-xs text-muted transition hover:border-accent hover:text-foreground disabled:opacity-50"
            >
              All collections
            </button>
          </div>
